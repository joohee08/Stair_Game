...
function shakeObject(gameObject, times = 8, distance = 5, duration = 50) {
    function shake(times, distance, duration) {
        if (times > 0) {
            // 좌우로 흔들림
            gameObject.x += times % 2 === 0 ? distance : -distance;
            // 다음 흔들림 호출
            gameObject.scene.time.delayedCall(duration, () => {
                shake(--times, distance, duration);
            });
        } else {
    ...
    // 원래 위치 저장
    if (typeof gameObject.originalX === 'undefined') {
...
export { itoStr, roundTo3D, ArrStrToInt, delElmFromArr, arraysEqual, addElmSort, isEmpty, 
shake_fn, shakeObject, setPointerCursor, shuffleArray, initializeNamedChildren  /* 나머지 함수 */ };